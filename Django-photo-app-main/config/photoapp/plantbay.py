    ...
    if uploaded_file != None:
        ...
        if store_file:
            ...
            if store_file is not None:
                ...
                User = get_user_model()
                if User == None:
                    st.error("Please loging to store files")
                else:
                    user = User.objects.first()  # replace with a valid user instance
                    st.write("File saved under user:" + user)

                    # create a new Photo object with submitter set to user
                    photo = Photo(title=result['prediction'], description=report_text1+report_text2+report_text3, image=stored_path, submitter=user)

                    # save the object to the database
                    photo.save()

                    # add tags to the object
                    photo.tags.add(result['prediction'])
